...
export function openInIINA(tabId, url, options = {}) {
  ...
  if (chrome.scripting?.executeScript) {
    ...
    chrome.tabs.executeScript(tabId, {
      code: `(${openIINA.toString()})(${JSON.stringify(baseURL)}, ${JSON.stringify(params)});`,
    });
  }
}
...
export function updateBrowserAction() {
  getOptions((options) => {
    if (options.iconAction === "clickOnly") {
      (chrome.action || chrome.browserAction).setPopup({ popup: "" });
    } else {
      (chrome.action || chrome.browserAction).setPopup({ popup: "popup.html" });
    }
  });
}
